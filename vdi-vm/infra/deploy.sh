...
# Fetch any Secret Manager secrets named vdi-vm-tfvars* and same them to .auto.tfvars files.
for secret in $(gcloud -q secrets list --filter=name~vdi-vm-tfvars- --format="value(name)"); do
    latest=$(gcloud secrets versions list ${secret} --sort-by=created --format='value(name)')
    dest="${secret/vdi-vm-tfvars-/}.auto.tfvars"
